...
public class ViewRightFoods extends RelativeLayout implements ViewBaseAction {
	...
	public ViewRightFoods(Context context, List<String> Foods) {
		super(context);
		int size = Foods.size();
	...
	public ViewRightFoods(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs, defStyle);
		init(context);
	}

	public ViewRightFoods(Context context, AttributeSet attrs) {
		super(context, attrs);
		init(context);
	}
...
//		mListView.setAdapter(adapter);
	    ...
	    List<String> shicais = new ArrayList<String>();
	    for(int i=0;i<items.length;i++)
	    {
	    	shicais.add(items[i]);
	    }
	    gridview.setAdapter(new GridViewAdapter(context,shicais));
